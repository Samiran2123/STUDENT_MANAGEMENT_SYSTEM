...
const pool = new Pool({
  ...
  password: process.env.DB_PASSWORD || '436823',
  port: process.env.DB_PORT || 5432,
});

async function runMigration() {
  try {
    const sql = fs.readFileSync(path.join(__dirname, 'config', 'erp_migration.sql'), 'utf8');
    console.log('Running ERP migration...');
    await pool.query(sql);
    console.log('Migration schema applied successfully.');

    // Execute dynamic full ERP database seeding
    const classesData = [
    ...
    // Academic year setup
    let ayRes = await pool.query("SELECT id FROM academic_years WHERE year_name = '2026-2027'");
    let academicYearId;
    ...
    const allClassesRes = await pool.query('SELECT id, name, degree, year_level FROM classes');
    ...
    console.log('Default ERP data seeded successfully.');
  } catch (err) {
