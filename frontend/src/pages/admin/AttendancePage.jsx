...
import { FiPlus, FiEdit, FiTrash2, FiCheckSquare, FiGrid, FiList } from 'react-icons/fi';
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
        <div>
          <h1 style={{ fontSize: '1.75rem', fontWeight: 800 }}>Attendance Tracker</h1>
          <p style={{ color: 'var(--text-muted)', fontSize: '0.9rem' }}>
            Track daily course attendance, bulk record sessions, and analyze presence logs.
          </p>
        </div>
      ...
      {/* Table */}
      {loading ? (
        <Spinner text="Fetching attendance logs..." />
      ) : attendance.length === 0 ? (
        <EmptyState title="No Attendance Logs" message="No presence records found matching your filters." />
      ) : (
                ...
                <tr key={rec.id} style={{ borderBottom: '1px solid var(--border-glass)' }}>
                  ...
                  <td style={{ padding: '14px 18px', textAlign: 'right' }}>
                    <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '8px' }}>
                      <button onClick={() => handleEditClick(rec)} style={{ padding: '6px', color: 'var(--secondary)', backgroundColor: 'rgba(6, 182, 212, 0.1)', borderRadius: 'var(--radius-sm)' }}>
                        <FiEdit />
                      </button>
                      <button onClick={() => handleDeleteClick(rec)} style={{ padding: '6px', color: 'var(--danger)', backgroundColor: 'var(--danger-bg)', borderRadius: 'var(--radius-sm)' }}>
                        <FiTrash2 />
                      </button>
                    </div>
                  </td>
